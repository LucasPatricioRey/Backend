...
export class ProductManagerDB {
    ...
    paginate = async( { stock, category }, { limit=10, page=1, sort } ) => {
        //! HANDLE ERRORS
        const optCategory=['accesorio', 'Otra cosa']
        
        if( stock !== undefined && isNaN( Number( stock ) ) ) throw `Must send a valid stock not: ${ stock }`
        if ( category !== undefined && !optCategory.includes( category ) ) throw `Must send a valid Category between 'accesorio', 'Otra cosa'. Not: ${ category }`
